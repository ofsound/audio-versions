...
import {
	...
	saveAudioFile,
} from "#/lib/version-compare/db";
import type { VersionCompareSnapshot } from "#/lib/version-compare/types";
import {
	EMPTY_SNAPSHOT,
	normalizeLoadedSnapshot,
} from "./version-compare-provider-hydration";

export type SnapshotUpdater = (
	current: VersionCompareSnapshot,
) => VersionCompareSnapshot;

export type SnapshotPersist = (next: VersionCompareSnapshot) => Promise<void>;
...
) => Promise<VersionCompareSnapshot>;

function hasLibraryContent(snapshot: VersionCompareSnapshot): boolean {
	return (
...
function mergeLegacySnapshot(
	local: VersionCompareSnapshot,
	cloud: VersionCompareSnapshot,
): VersionCompareSnapshot {
...
function needsMediaMigration(snapshot: VersionCompareSnapshot): boolean {
...
export function useVersionCompareSnapshotState(cloudUserId?: string) {
	const [snapshot, setSnapshot] =
		useState<VersionCompareSnapshot>(EMPTY_SNAPSHOT);
			...
			.catch((loadError) => {
				...
				setError(
					loadError instanceof Error
						? loadError.message
						: "Version Compare could not load the local workspace.",
				);
				setReady(true);
			});
						...
						if (!cancelled) {
							setError(
								syncError instanceof Error
									? syncError.message
									: "Version Compare could not receive cloud changes.",
							);
						}
					});
			}, 80);
			...
			try {
				...
				setError(
					persistError instanceof Error
						...
						: "Version Compare could not save the latest changes.",
				);
				throw persistError;
			}
		},
