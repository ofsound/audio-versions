...
interface UseAudioVersionsSelectorsOptions {
...
export function useAudioVersionsSelectors({
	snapshot,
}: UseAudioVersionsSelectorsOptions) {
	...
	const annotationsByFileId = useMemo(() => {
		const entries = new Map<string, AudioVersionsSnapshot["annotations"]>();
