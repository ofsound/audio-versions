...
import { createDefaultUiSettings } from "#/lib/version-compare/types";
...
vi.mock("./rich-text-editor", () => ({
	RichTextEditor: ({ focusId }: { focusId?: string }) => (
		<div data-testid="rich-text-editor" data-version-compare-editor={focusId} />
	),
}));
...
vi.mock("#/providers/version-compare-provider", () => ({
	useVersionCompare: () => ({
		ready: true,
		getSongById,
		getSongAudioFiles,
...
describe("SongWorkspace", () => {
	...
	it("renders the journal editor without an outer journal shell", () => {
		...
		const journalEditor = document.querySelector(
			'[data-version-compare-editor="journal"]',
		);
