...
import {
	...
	type RichTextDoc,
	type Song,
	type VersionCompareSettings,
	type VersionCompareSnapshot,
} from "./types";

interface VersionCompareDB extends DBSchema {
	songs: {
		key: string;
		value: Song;
	};
...
let dbPromise: Promise<IDBPDatabase<VersionCompareDB>> | null = null;

type LegacyAudioFileRecord = AudioFileRecord & {
	masteringNote?: RichTextDoc | null;
};

function getDb(): Promise<IDBPDatabase<VersionCompareDB>> {
	dbPromise ??= openDB<VersionCompareDB>(DB_NAME, DB_VERSION, {
...
export async function closeVersionCompareDbForTests(): Promise<void> {
	if (!dbPromise) {
		return;
	}
...
export async function loadSnapshot(): Promise<VersionCompareSnapshot> {
