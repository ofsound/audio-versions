...
function deleteVersionCompareDatabase() {
	return new Promise<void>((resolve, reject) => {
		const request = indexedDB.deleteDatabase(DB_NAME);
		request.onsuccess = () => resolve();
		request.onerror = () => reject(request.error);
		request.onblocked = () =>
			reject(new Error("Version Compare test database is blocked."));
	});
}
...
function openLegacyVersionCompareDatabase(
	version: number,
): Promise<IDBDatabase> {
...
const baseSettings: VersionCompareSettings = {
	recents: ["song-1"],
...
describe("version-compare db cascade helpers", () => {
	beforeEach(async () => {
		vi.resetModules();
		const db = await loadDbModule();
		await db.closeVersionCompareDbForTests();
		await deleteVersionCompareDatabase();
		vi.resetModules();
	});
	...
	it("upgrades legacy annotation colors and missing session dates in v3", async () => {
		const legacyDb = await openLegacyVersionCompareDatabase(2);
