...
- Preserve compatibility with the shared Supabase rows, rich-text JSON, waveform JSON, annotation semantics, and signed-media API.
...
- Encode edited notes using the existing shared rich-text compatibility layer; do not invent a native-only storage format.
